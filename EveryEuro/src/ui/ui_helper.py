...
def create_menu_bar(root, get_data_from_file, table_all_months_planned, table_all_months_receivedspent):
    """ Creating menu bar in the top part on the window. """
    top = tk.Menu(root)
    root.config(menu=top)
    file = tk.Menu(top, tearoff=False)
    file.add_command(label='Open file', command=get_data_from_file, underline=0)
    file.add_command(label='Save to file',
...
def show_year_summary(table_all_months_receivedspent, year_overview_window):
    ...
    create_and_place_year_overview_number_items(frame_summary, sum_total_income, sum_total_rent,
...
def create_comments_year_overview(frame_summary, count_months_rent_above_35_per_cent_of_income,
    ...
    if spending_percentage_of_income >= 50:
        ...
        label_spending_too_much.grid(row=row_number, column=1, sticky="w")
        row_number += 1
